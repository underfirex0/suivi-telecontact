...
import { AlertTriangle, CheckCircle2, TrendingUp, FileWarning } from "lucide-react";
...
export function ImportDiffView({ diff }: { diff: ImportDiff }) {
  const { nbNouveaux, nbSoldes, nbPartiels, montantTotalRegle } = diffKpis(diff);
    ...
    <div>
      <div className="mb-6 grid grid-cols-4 gap-3.5">
        ...
        <div className="rounded-xl border border-border bg-surface p-4 shadow-card">
          ...
          <div className="mt-1.5 font-mono text-[20px] font-bold text-ink">{formatMontant(montantTotalRegle)}</div>
        </div>
      </div>
        ...
        <div className="mb-6">
          ...
          <div className="overflow-hidden rounded-xl border border-border bg-surface shadow-card">
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-surface-2">
                  {["Client", "N° facture", "Ajouté", "Ancien reçu", "Nouveau total", "Solde après", "Résultat"].map(
                    (h) => (
                      <th
                        key={h}
                        className="border-b border-border px-3 py-2 text-left text-[10.5px] font-semibold uppercase tracking-wide text-ink-2"
      ...
      {diff.nouveaux.length === 0 && diff.paiementsExistants.length === 0 && diff.anomalies.length === 0 && (
