...
export function exportDossiersToCsv(dossiers: Dossier[], profiles: Profile[]) {
  ...
  const headers = [
    ...
    "Montant reçu",
    "Reste dû",
    "% temps visibilité consommé",
    "% payé",
    "Niveau (écart temps/payé)",
    "Promesse non tenue",
    "Jours sans action",
    "Abandonné",
    "Raison abandon",
  ...
  const rows = dossiers.map((d) => {
    ...
    return [
      ...
      formatMontant(d.montant_recu),
      d.montant_facture != null ? formatMontant(d.montant_facture - d.montant_recu) : "",
      a.pctTemps != null ? `${Math.round(a.pctTemps)}%` : "",
      a.pctPaye != null ? `${Math.round(a.pctPaye)}%` : "",
      String(a.niveau),
      a.promesseRompue ? "Oui" : "Non",
      a.joursSansAction != null ? String(a.joursSansAction) : "",
      d.abandonne_at ? "Oui" : "Non",
      d.abandonne_raison ?? "",
      d.date_paiement ?? "",
